...
export class RegexStrategy extends ValidationStrategy {
  async execute() {
    ...
    const answers = await inquirer.prompt([
      ...
      {
        name: 'flags',
        message: 'Enter the flags for the regular expression (leave empty for no flags):',
        type: 'input',
      },
      {
        name: 'invertMatch',
      ...
      },
      {
        ...
        message: 'Should the regular expression be tested against the trimmed value (true/false)?',
        type: 'confirm',
      },
      {
        name: 'allowEmptyString',
        message: 'Should an empty string be allowed as valid regardless of the pattern (true/false)?',
        type: 'confirm',
      },
      {
        name: 'message',
