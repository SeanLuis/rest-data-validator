...
export class PasswordStrategy extends ValidationStrategy {
  async execute() {
    ...
    const answers = await inquirer.prompt([
      ...
      {
        name: 'minLength',
        message: 'Enter the minimum length of the password (leave empty for no minimum):',
        type: 'input',
      },
      {
        name: 'maxLength',
        message: 'Enter the maximum length of the password (leave empty for no maximum):',
        type: 'input',
      },
      {
        name: 'regexPattern',
        message: 'Enter a regular expression pattern to validate the password against (leave empty for no pattern):',
        type: 'input',
      },
      {
        name: 'mustContainLowercase',
        message: 'Should the password contain at least one lowercase letter (true/false)?',
        type: 'confirm',
      },
      {
        name: 'mustContainUppercase',
        message: 'Should the password contain at least one uppercase letter (true/false)?',
        type: 'confirm',
      },
      {
        name: 'mustContainNumber',
        message: 'Should the password contain at least one number (true/false)?',
        type: 'confirm',
      },
      {
        name: 'mustContainSpecialCharacter',
        message: 'Should the password contain at least one special character (true/false)?',
        type: 'confirm',
      },
      {
        name: 'message',
      ...
      property.addDecorator({
        ...
        arguments: [
          `{
            ...
            ${answers.regexPattern ? `regexPattern: /${answers.regexPattern}/,` : ''}
