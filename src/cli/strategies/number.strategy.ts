...
export class NumberStrategy extends ValidationStrategy {
  async execute() {
    ...
    const answers = await inquirer.prompt([
      ...
      {
        name: 'precision',
        message: 'Enter the maximum number of digits allowed after the decimal point (leave empty for any number of digits):',
        type: 'input',
      },
      {
        name: 'divisibleBy',
        message: 'Enter the value that the number must be divisible by (leave empty for any value):',
        type: 'input',
      },
      {
        name: 'notEqualTo',
        message: 'Enter an array of values that the number must not equal to (comma separated, leave empty for any value):',
        type: 'input',
      },
      {
        name: 'equalTo',
        message: 'Enter an array of values that the number can specifically equal to (comma separated, leave empty for any value):',
        type: 'input',
      },
      {
        name: 'message',
      ...
      property.addDecorator({
        ...
        arguments: [
          `{
            ...
            ${answers.notEqualTo ? `notEqualTo: [${answers.notEqualTo.split(',').map((value: string) => value.trim())}],` : ''}
            ${answers.equalTo ? `equalTo: [${answers.equalTo.split(',').map((value: string) => value.trim())}],` : ''}
